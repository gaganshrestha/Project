...
class Permalink(Handler):
        def get(self, blog_id):
                ...
                key = 'parma'
                query_time_key='parma_query_time'
                blog = memcache.get(key)
                query_time = memcache.get(query_time_key)
                if blog is None:
                        ...
                        query_time = str(int(time.time() - query_time))
                                
                if self.format == 'html':
                        self.render("list.html", blogs = [blog], age=setage(query_time))
                else:
                        time_fmt = '%b %d, %Y'
...
class ListHandler(Handler):
    def get(self):
        ...
        if self.format == 'html':
                self.render("list.html",blogs=blogs,age=setage(query_time))
